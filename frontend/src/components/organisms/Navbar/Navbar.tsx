...
export const Navbar: React.FC<NavbarProps> = ({ className = '' }) => {
  ...
  const categoryItems: CategoryItem[] = [
    { label: 'Breaking News', href: '/breaking-news', variant: 'default', active: false },
    ...
    <nav className={`bg-white border-b border-gray-200 w-full ${className}`}>
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:block py-2 border-b border-gray-100">
          <NavigationMenu items={mainNavItems} className="justify-start space-x-6" />
        </div>

        {/* Category Navigation - Responsive */}
        <div className="py-2">
          <CategoryNav items={categoryItems} className="justify-start gap-2 text-sm" />
        </div>
