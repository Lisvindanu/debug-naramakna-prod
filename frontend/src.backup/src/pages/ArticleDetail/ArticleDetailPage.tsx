...
export const ArticleDetailPage: React.FC<ArticleDetailPageProps> = ({ articleId, articleSlug }) => {
  ...
  const fetchArticleById = async (id: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts/${id}`, {
        credentials: 'include'
      });
  ...
  const fetchArticleBySlug = async (slug: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts/slug/${slug}`, {
        credentials: 'include'
      });
  ...
  const fetchRelatedArticles = async (excludeId: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts?limit=6&exclude=${excludeId}`, {
        credentials: 'include'
      });
  ...
  const fetchRelatedArticlesBySlug = async (excludeSlug: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts?limit=6&excludeSlug=${excludeSlug}`, {
        credentials: 'include'
      });
  ...
  const fetchComments = async (postId: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts/${postId}/comments`, {
        credentials: 'include'
      });
  ...
  const fetchCommentsBySlug = async (slug: string) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts/slug/${slug}/comments`, {
        credentials: 'include'
      });
  ...
  const trackView = async (postId: string) => {
    try {
      await fetch('https://dev.naramakna.id/api/analytics/track', {
        method: 'POST',
        headers: {
  ...
  const handleAddComment = async (content: string) => {
    ...
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/posts/${article.id}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
