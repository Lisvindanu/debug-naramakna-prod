...
const ArticleWriterPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPopularTags = async () => {
      try {
        const response = await fetch('https://dev.naramakna.id/api/category/popular-tags');
        if (response.ok) {
  ...
  const imageHandler = useCallback(() => {
    ...
    input.onchange = async () => {
      ...
      if (file) {
        ...
        try {
          const response = await fetch('https://dev.naramakna.id/api/writer/upload-image', {
            method: 'POST',
            credentials: 'include',
            body: formData
          });
  ...
  const autoSave = useCallback(async () => {
    ...
    try {
      ...
      const url = isEditMode 
        ? `https://dev.naramakna.id/api/writer/articles/${editId}`
        : 'https://dev.naramakna.id/api/writer/articles';
  ...
  useEffect(() => {
    ...
    const loadArticle = async () => {
      try {
        ...
        const response = await fetch(`https://dev.naramakna.id/api/writer/articles/${editId}`, {
          credentials: 'include'
        });
