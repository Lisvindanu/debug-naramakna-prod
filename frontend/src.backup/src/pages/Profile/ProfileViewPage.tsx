...
const getImageUrl = (imagePath: string | null) => {
  ...
  if (imagePath.startsWith('/uploads/')) {
    return `https://dev.naramakna.id${imagePath}`;
  }
  
  // Otherwise, assume it's a relative path and prepend backend URL
  return `https://dev.naramakna.id/uploads/${imagePath}`;
};
    ...
    const fetchUserArticles = async () => {
      ...
      try {
        ...
        if (isOwnProfile) {
          ...
          const publishedResponse = await fetch(`https://dev.naramakna.id/api/content/author/${authorId}?status=publish&limit=20`, {
            credentials: 'include'
          });
          
          console.log('🔍 Fetching pending posts...');
          const pendingResponse = await fetch(`https://dev.naramakna.id/api/content/author/${authorId}?status=pending&limit=20`, {
            credentials: 'include'
          });
          
          console.log('🔍 Fetching draft posts...');
          const draftResponse = await fetch(`https://dev.naramakna.id/api/content/author/${authorId}?status=draft&limit=20`, {
            credentials: 'include'
          });
          ...
          const apiUrl = `https://dev.naramakna.id/api/content/author/${authorId}?status=publish&limit=20`;
