...
export const AdminPolling: React.FC = () => {
  ...
  const fetchPolls = async () => {
    try {
      const response = await fetch('https://dev.naramakna.id/api/polling/admin/all?limit=50', {
        headers: {
      ...
      if (data.success) {
        ...
        const fallbackResponse = await fetch('https://dev.naramakna.id/api/polling/active?limit=50', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const fetchTrendingPosts = async () => {
    try {
      const response = await fetch('https://dev.naramakna.id/api/polling/trending-candidates?limit=10', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchArticles = async () => {
    try {
      const response = await fetch('https://dev.naramakna.id/api/content/posts-with-views?limit=50&status=publish', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const createPoll = async () => {
    ...
    setCreating(true);
    try {
      const response = await fetch('https://dev.naramakna.id/api/polling/create', {
        method: 'POST',
        headers: {
  ...
  const generatePollFromPost = async (post: TrendingPost) => {
    setCreating(true);
    try {
      const response = await fetch('https://dev.naramakna.id/api/polling/generate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const closePoll = async (pollId: string) => {
    ...
    try {
      const response = await fetch(`https://dev.naramakna.id/api/polling/${pollId}/close`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
