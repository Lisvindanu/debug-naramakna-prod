...
const SuperAdminDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [usersRes, userStatsRes, contentStatsRes, categoriesRes, postsRes, pendingPostsRes] = await Promise.all([
        fetch('https://dev.naramakna.id/api/users', {
          method: 'GET',
          credentials: 'include',
          headers: {
        ...
        fetch('https://dev.naramakna.id/api/users/stats', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
        ...
        fetch('https://dev.naramakna.id/api/content/stats', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
        ...
        fetch('https://dev.naramakna.id/api/content/categories?minCount=0', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
        ...
        fetch(`https://dev.naramakna.id/api/content/posts-with-views?limit=${pagination.itemsPerPage}&page=${pagination.currentPage}&${new URLSearchParams(
        ...
        fetch('https://dev.naramakna.id/api/approval/pending', {
          method: 'GET',
          credentials: 'include',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const suspendUser = async (userId: number) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/admin/users/${userId}/suspend`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteUser = async (userId: number) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/admin/users/${userId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  const deleteArticle = async (articleId: number) => {
    try {
      const response = await fetch(`https://dev.naramakna.id/api/content/admin/articles/${articleId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
